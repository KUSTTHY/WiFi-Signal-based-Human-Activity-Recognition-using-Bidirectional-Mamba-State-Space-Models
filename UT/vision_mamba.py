...
class DepthwiseSeparableConv1D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.depthwise = nn.Conv1d(in_channels, in_channels * kernel_size, kernel_size, groups=in_channels // 30, padding=kernel_size // 2)
        # 逐点卷积
...
class FusionModel(nn.Module):
    def __init__(self):
        ...
        self.vision_model = VisionMamba(
        depth=8,
        embed_dim=90,  # 确保此参数与模型要求匹配
        channels=250,  # ConvNet 输出的通道数
        num_classes=7,  # 根据类别数设置
        if_cls_token=True,  # 拼不拼clstoken
        use_double_cls_token=True
    )
        self.DW = DepthwiseSeparableConv1D(in_channels=90, out_channels=90, kernel_size=5)
